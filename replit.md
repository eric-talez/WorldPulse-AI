...
- Optional Cesium env (Earth realism): `VITE_CESIUM_ION_TOKEN` enables Bing Maps Aerial with Labels (asset 3) + Cesium World Terrain (asset 1) for a Google-Earth-class globe; falls back to ESRI World Imagery + ESRI labels overlay when missing
- Optional admin env: `ADMIN_WALLETS` (comma-separated lowercase wallet addresses) auto-elevates those wallet sessions to admin without needing the `ADMIN_EMAIL`/`ADMIN_PASSWORD` login flow
...
- Earth realism stack (chosen for the "photo-of-Earth" look without ion or paid tiles): `globe.enableLighting = true` + `dynamicAtmosphereLighting` for a real sun-driven day/night terminator, base ESRI imagery layer dimmed on the night side via `nightAlpha = 0.45` so continents stay readable, a NASA GIBS `VIIRS_CityLights_2012` overlay added with `dayAlpha = 0` / `nightAlpha = 1` so the night hemisphere lights up with city lights instead of going black, and a third faint cloud overlay from NASA GIBS `MODIS_Terra_CorrectedReflectance_TrueColor` (pinned to a fixed snapshot date — task explicitly excludes time-aware tiling) at `alpha = 0.3` with `dayAlpha = 0.35` / `nightAlpha = 0.18` so clouds read as the brightest pixels without crushing continents or competing with city lights. Atmosphere `brightnessShift` (0.07) and fog density (1.0e-4) were re-tuned down a notch so the cloud layer doesn't wash out the rim glow. Lighting is Earth-only — Moon/Mars stay on flat shading so the whole airless body is visible at once.
